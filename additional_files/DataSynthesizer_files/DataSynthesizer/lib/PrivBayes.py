...
def permute_flip(epsilon, mutual_info_list, parents_pair_list, attr_to_is_binary, num_tuples, num_attributes):
    ...
    mi_array = mutual_info_list
    maxarray = max(mi_array)
    shuffle = list(range(len(mi_array)))
    random.shuffle(shuffle)
    dist_array = np.zeros(len(mi_array))
    for i in shuffle:
        p = np.exp((delta_array[i]/2)*(mi_array[i]-maxarray))

...
def MST(data, epsilon, delta, outcome, admissible, protected):
    rho = cdp_rho(epsilon, delta)
    sigma = np.sqrt(3/(2*rho))
    cliques = [(col,) for col in data.domain]
    log1 = measure(data, cliques, sigma)
    data, log1, undo_compress_fn = compress_domain(data, log1)
    cliques = select(data, rho/3.0,log1, outcome, admissible,protected)
...
def select(data, rho, measurement_log,outcome,admissible, protected, cliques=[] ):
    engine = FactoredInference(data.domain, iters=1000)
    est = engine.estimate(measurement_log)
    r = len(data.domain.attrs)
    r = (r*r -r)/2
    sigma = math.sqrt(rho/(2*r))
    ...
    for a, b in candidates:
        ...
        weights[a,b] = max(np.linalg.norm(x - xhat, 1) + np.random.normal(0,sigma),0)
    ...
    heapq.heappush(heap,(0,id(T),T,ds))
    while(len(heap)>0):
        W,iden,T,ds = heapq.heappop(heap)
        candidates_current = [e for e in candidates if not ds.connected(*e)]

        if(T.number_of_edges() == T.number_of_nodes() -1):
            #print(list(T.edges))
            #nx.draw(T)
            return list(T.edges)
        for e in candidates_current:

            W_current = W + weights[e]
            T_current = copy.deepcopy(T)
            T_current.add_edge(*e)
            ds_current = copy.deepcopy(ds)
            ds_current.union(*e)
            bad_path = False
            for i in protected:
                for j in outcome:
                    if(nx.has_path(T_current,i,j)):
                        path = nx.shortest_path(T_current,i,j)
                        pathset = set(path)
                        admissibleset = set(admissible)
                        bad_path = bad_path or (len(pathset.intersection(admissibleset)) == 0)

            if(not bad_path):
                exists = False
                for i in heap:
                    exists = exists or nx.is_isomorphic(T,i[2])
                if(not exists):
                    heapq.heappush(heap, (-1*W_current,id(T_current),T_current,ds_current))
